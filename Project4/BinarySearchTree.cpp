...
// Course: CSC237
// Description: Binary Tree ADT implemented with TreeNode linked structures

/*!	\file BinarySearchTree.cpp 
 * *	\brief Custom data structure for a binary search tree
 * */
...
{if (root==NULL) { // Empty Tree
 ...
 while(t!=NULL) {
   if (t->info==data) {// data already in Tree
     //UPDATE: If the data matches, increment the counter
     t->count = t->count + 1;
     return(0);
   } 
...
// Search for Element in Tree
// Assumes == is defined for treeEltType
// Returns Ptr to count if Found, NULL otherwise
template <typename treeEltType>
	void BinaryTree<treeEltType>::decrementNode(const treeEltType &key)
{TreeNode<treeEltType> *t=root;
 while (t && t->info != key)
   if (key < t->info) 
     t=t->left;
...
///////////////////////////////////////////////////////////////////////////////
template <typename treeEltType>
   treeEltType &BinaryTree<treeEltType>::retrieveCount(const treeEltType &key) 
...
{TreeNode<treeEltType> *nodeWithData,*nodeToDelete,*t=root,*trailT=NULL;
 ...
 if (!(nodeWithData->left) && !(nodeWithData->right)) {
  ...
  //UPDATE: swaps the count for the node that is being removed
...
{if (t) {
  printInorder(t->left);
  //UPDATE: If count greater than 1, print with parathesis
  if(t->count > 1)
...
{queue<TreeNode<treeEltType> *> Q;
 ...
 if (root) {
   //UPDATE: If root's counter is greater than 1, print with parenthesis
   if(root->count > 1)
 ...
 while (!Q.empty()) {
  ...
  if (t==dummy) {
   ...
   //UPDATE: If t's count is greater than 1, print with parenthesis
   if(t->count > 1)
      cout << t->info << " (" << t->count << ") " << endl;
   else
      cout << t->info << " ";
